<%@ Page Language="vb" AutoEventWireup="true" CodeFile="Default.aspx.vb" Inherits="MasterDetailSetup" %>
	...
	<dxwgv:ASPxGridView ID="grid" ClientInstanceName="grid" runat="server" KeyFieldName="Id" OnCustomCallback="grid_CustomCallback" OnCustomDataCallback="grid_CustomDataCallback">
		...
		<Columns>
			...
			<dxwgv:GridViewDataColumn FieldName="IsChecked" VisibleIndex="1">
				<DataItemTemplate>
					<input type="checkbox" <%#Eval("HtmlChecked")%> onclick="<%#GetMasterGridCallBack(Container.VisibleIndex, CInt(Fix(Container.KeyValue)))%>" />
				</DataItemTemplate>
			</dxwgv:GridViewDataColumn>
		</Columns>
		<%-- EndRegion --%>
			...
			<DetailRow>
				<dxwgv:ASPxGridView ID="detailGrid" runat="server" 
					...
					<Columns>
						...
						<dxwgv:GridViewDataColumn FieldName="IsChecked" VisibleIndex="1">
							<DataItemTemplate>
								<input type="checkbox" <%#Eval("HtmlChecked")%> onclick="<%#GetGridDataCallBack(CInt(Fix(Container.KeyValue)))%>" />
							</DataItemTemplate>
						</dxwgv:GridViewDataColumn>
					</Columns>
					<%-- EndRegion --%>
